 ...
 *   - Valeur de stock (quantité × prix de vente)  [marge = TODO, pas de coût d'achat]
 */
...
export async function getPilotageToday(
  ...
  try {
    const [cmdRes, cashmagRes, stockRes, stockoutRes] = await Promise.all([
      ...
      (() => {
        let q = sb.from("stock_par_depot").select("quantite, prix_vente");
